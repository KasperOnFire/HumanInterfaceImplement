...
public class TextualServer implements ConnectionHandler {

    public static void main(String[] args) {
        Server server = new Server(new TextualServer(), 5665);
        server.run();
    }

    @Override
    public void handleConnection(SocketConnection conn) {
        TextualInterface ti = new QuestionSide(new TextualConnectionImp(conn));
